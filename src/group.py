...
def projBoundedSimplex(x, alpha=0., z=1., verbose=0):
    ...
    if alpha < float(z) / x.size:
        ...
        if verbose:
            print "impossible alpha. reset to %.5f" % (alpha)
    y = np.sort(x)[::-1]
    n = y.size
    sumY = 0.
    t = None
...
def projector_group_constraint(canonical_dict=None, lro_dict=None, tucker_dict=None, dimnums=[0], vect=True, activeModes=None):
...
def hessian_mv_group_constraint(x, multipliers, canonical_dict=None, lro_dict=None, tucker_dict=None, dimnums=[0], vect=True):
    assert (lro_dict is not None) or ((tucker_dict is not None) and (lro_dict is not None)),\
        "group_constraint: lro and/or tucker parts must be specified"
    lFlag = False
    Ch = None
...
def func_group_constraint(multipliers, pcum, pmin, canonical_dict=None, lro_dict=None, tucker_dict=None, dimnums=[0], vect=True):
    assert (lro_dict is not None) or ((tucker_dict is not None) and (lro_dict is not None)),\
        "group_constraint: lro and/or tucker parts must be specified"
    lFlag = False
    if lro_dict is not None: # always True
