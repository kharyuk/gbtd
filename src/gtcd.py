...
def swing(x, k):
    return  np.transpose(x, range(1, k+1) + [0] + range(k+1, x.ndim))
...
def fastSVD(x, R=None, tol=None, scale=1.5):
    ...
    if m >= scale*n:
        tmp = _np.dot(x.T, x)
        _, s, vt = _np.linalg.svd(tmp)
        if tol is not None:
            csumdiff = _np.diff(_np.cumsum(s))
        ...
        if R is not None:
            ...
            u = u[:, :Rs]
        s = s**0.5
        vt = _np.dot(u.T, x).T / s
    else:
        u, s, vt = _np.linalg.svd(x)
        if tol is not None:
            csumdiff = _np.diff(_np.cumsum(s**2.))
            Rtol = csumdiff[csumdiff >= tol].sum()
        if R is not None:
            Rs = R
            if tol is not None:
...
def computeE(L, iden=_SQRT_CLOBAL):
    R = len(L)
    M = int(sum(L))
    E = np.zeros([R, M])
    cind = 0
    for t in xrange(R):
        ...
        if iden:
            coef = 1./np.sqrt(L[t])
        E[t, cind : cind + int(L[t])] = coef*np.ones(int(L[t]))
        cind += int(L[t])
    return E

def recover(n, canonical_dict=None, lro_dict=None, tucker_dict=None):
    ...
    if lro_dict is not None:
        ...
        if canonical_dict is not None:
            tmp = np.zeros([n[1], partCP.shape[1] + partB.shape[1]])
    ...
    for m in xrange(Rt):
        ...
        for k in xrange(d):
            if tucker_dict is not None:
                partGm = prodTenMat(partGm, A[k][:, int(np.sum(r[:m, k])) : int(np.sum(r[:m+1, k]))], k)
            if m == 0:
                if k > 1:
                    if canonical_dict is not None:
                        tmp = C[k].copy()
                    if lro_dict is not None:
                        ...
                        if canonical_dict is not None:
                            tmp1 = tmp.copy()
                            tmp = np.zeros([n[k], tmp1.shape[1] + partB.shape[1]])
    ...
    if lro_dict is not None:
        ...
        if canonical_dict is not None:
            tmp1 = tmp.copy()
            tmp = np.zeros([n[0], tmp1.shape[1] + partB.shape[1]])
            tmp[:, :tmp1.shape[1]] += tmp1
            tmp[:, tmp1.shape[1]:] += partB
            del tmp1, partB
        else:
...
def initializeCBT(n, canonical_param=None, lro_param=None, tucker_param=None, rtype='uniform', normalize=True):
    '''
    Tucker blocks
        r = rank parameters
    Canonical block
    ...
    '''
    assert (canonical_param is not None) or (lro_param is not None) or (tucker_param is not None), "initializeCBT(): Empty input!"
    d = len(n)
    ...
    if tucker_param is not None:
        ...
        Rt = r.shape[0]
    if rtype == 'normal':
        mgen = lambda x: np.random.normal(loc=0., scale=1., size=x)
    elif rtype == 'beta':
        mgen = lambda x: np.random.beta(a=2, b=1, size=x)
    elif rtype == 'binomial':
        mgen = lambda x: np.random.binomial(n=2, p=0.5, size=x)
    elif rtype == 'chisquare':
        mgen = lambda x: np.random.chisquare(df=10, size=x)
    elif rtype == 'dirichlet':
        mgen = lambda x: np.random.dirichlet(alpha=2., size=x)
    elif rtype == 'exponential':
        mgen = lambda x: np.random.exponential(scale=2., size=x)
    elif rtype == 'f':
        mgen = lambda x: np.random.f(dfnum=2., dfden=1., size=x)
    elif rtype == 'gamma':
        mgen = lambda x: np.random.gamma(shape=3., scale=1., size=x)
    elif rtype == 'geometric':
        mgen = lambda x: np.random.geometric(p=0.8, size=x)
    elif rtype == 'gumbel':
        mgen = lambda x: np.random.gumbel(loc=0., scale=1., size=x)
    elif rtype == 'hypergeometric':
        mgen = lambda x: np.random.hypergeometric(ngood=5, nbad=5, nsample=5, size=x)
    elif rtype == 'laplace':
        mgen = lambda x: np.random.laplace(loc=0., scale=1., size=x)
    elif rtype == 'logistic':
        mgen = lambda x: np.random.logistic(loc=0., scale=1., size=x)
    elif rtype == 'lognormal':
        mgen = lambda x: np.random.lognormal(mean=0., sigma=1., size=x)
    elif rtype == 'logseries':
        mgen = lambda x: np.random.logseries(p=0.5, size=x)
    elif rtype == 'multinomial':
        mgen = lambda x: np.random.multinomial(n=5., pvals=5., size=x)
    elif rtype == 'negative_binomial':
        mgen = lambda x: np.random.negative_binomial(n=5., p=0.5, size=x)
    elif rtype == 'pareto':
        mgen = lambda x: np.random.pareto(a=5., size=x)
    elif rtype == 'poisson':
        mgen = lambda x: np.random.poisson(lam=0.5, size=x)
    elif rtype == 'power':
        mgen = lambda x: np.random.power(a=5., size=x)
    elif rtype == 'rayleigh':
        mgen = lambda x: np.random.rayleigh(scale=1., size=x)
    elif rtype == 'standart_cauchy':
        mgen = lambda x: np.random.standard_cauchy(size=x)
    elif rtype == 'standart_t':
        mgen = lambda x: np.random.standard_t(df=5., size=x)
    elif rtype == 'beta':
        mgen = lambda x: np.random.triangular(left=-1., mode=0., right=-1, size=x)
    elif rtype == 'uniform':
        mgen = lambda x: np.random.uniform(low=0, high=0.5, size=x)
    elif rtype == 'vonmises':
        mgen = lambda x: np.random.vonmises(mu=0., kappa=1., size=x)
    elif rtype == 'wald':
        mgen = lambda x: np.random.wald(mean=0., scale=1., size=x)
    elif rtype == 'weibul':
        mgen = lambda x: np.random.weibull(a=5., size=x)
    elif rtype == 'zipf':
        mgen = lambda x: np.random.zipf(a=5., size=x)
    else:
    ...
    Ares = []
    Gres = []
    for k in xrange(max(Rt, d)):
        if k < d:
            if tucker_param is not None:
                shapeAk = [n[k], int(np.sum(r[:, k]))]
                Ares.append(mgen(shapeAk))
                Ares[k] = Ares[k].astype(np.double)
                if normalize:
            ...
            if lro_param is not None:
                if k < P:
                    if (fmc is not None) and (fmc[k] is not None):
                        indL = int(sum(L[:fmc[k][1]]))
                        tmpBk = [
                            mgen([n[k], fmc[k][0]]).astype(np.double),
                            mgen([indL, fmc[k][0]]).astype(np.double)
                        ]
                        if fmc[k][1] < Rl:
                            tmpBk.append( mgen([n[k], M-indL]) )
                        else:
                            tmpBk.append(None)
                        if normalize:
                        ...
                        tmpBk = mgen([n[k], M]).astype(np.double)
                        if normalize:
                            tmpBk /= np.linalg.norm(tmpBk)
                else:
                    tmpBk = mgen([n[k], Rl]).astype(np.double)
                    if normalize:
                        tmpBk /= np.linalg.norm(tmpBk)
                Bres.append(tmpBk)
            if canonical_param is not None:
                shapeCk = [n[k], Rc]
                Cres.append(mgen(shapeCk))
                Cres[k] = Cres[k].astype(np.double)
                if normalize:
                    Cres[-1] /= np.linalg.norm(Cres[-1])#, axis=0, keepdims=True)
        if k < Rt:
            Gres.append(mgen(r[k, :]))
            Gres[k] = Gres[k].astype(np.double)
            if normalize:
                Gres[-1] /= np.linalg.norm(Gres[-1])
    return Cres, Bres, Ares, Gres

def fcore2fvec(n, canonical_dict=None, lro_dict=None, tucker_dict=None):
    assert (canonical_dict is not None) or (lro_dict is not None) or (tucker_dict is not None), "fcore2fvec(): Empty input!"
    d = len(n)
    indC = 0
    sizeC = 0
    indB = 0
    ...
    if lro_dict is not None:
        ...
        if P > 0:
            if fmc is not None:
                ...
                sizeB += M*int(np.sum(n[:P]))
        if P < d:
            sizeB += Rl*int(np.sum(n[P:]))
...
def fvec2fcore(x, n, canonical_dict=None, lro_dict=None, tucker_dict=None, full_result=True):
    assert (canonical_dict is not None) or (lro_dict is not None) or (tucker_dict is not None), "fvec2fcore(): Empty input!"
    d = len(n)
    indC = 0
    sizeC = 0
    indB = 0
...
def jacTCD_matvec(x, Th, canonical_dict=None, lro_dict=None, tucker_dict=None, return_vector=False, sqrt=_SQRT_CLOBAL):
    return gradientTCD(x, Th, canonical_dict, lro_dict, tucker_dict, return_vector, sqrt)

def gradientTCD(x, Th, canonical_dict=None, lro_dict=None, tucker_dict=None, return_vector=False, sqrt=_SQRT_CLOBAL):
    #, Th, C, A, G, L, r):
    d = Th.ndim
    n = Th.shape
    ...
    for k in xrange(max(d, Rt)):
        if k < d:
            if cFlag:
                #Cres[k] = np.zeros(C[k].shape)
                Cres[k] = np.zeros(Ch[k].shape)
            if lFlag:
                if (k < P) and (fmc is not None) and (fmc[k] is not None):
                    Brk = []
                    indL = int(sum(L[:fmc[k][1]]))
                    Brk.append(np.zeros([n[k], fmc[k][0]]))
                    Brk.append(np.zeros([indL, fmc[k][0]]))
                    if indL < M:
                        Brk.append(np.zeros([n[k], M-indL]))
                    else:
                        Brk.append(None)
                    Bres[k] = Brk
                else:
                    #Bres[k] = np.zeros(B[k].shape)
                    Bres[k] = np.zeros(Bh[k].shape)
            if tFlag:
                #Ares[k] = np.zeros(A[k].shape)
                Ares[k] = np.zeros(Ah[k].shape)
        if k < Rt:
            if tFlag:
                #Gres[k] = np.zeros(G[k].shape)
                Gres[k] = np.zeros(Gh[k].shape)
                    ...
                    for k in xrange(d):
                        ...
                        for p in xrange(d):
                            ...
                            if cFlag and lFlag and (m1 == 0) and (m2 == 0) and (s2 == 0):
                                if p < P:
                                    ...
                                    if sqrt:
                                        tmp = np.dot(np.tile(Bh[p][:, s1:s1+1]/ np.sqrt(L[s1]), [1, L[s1]]).T , Ch[p]) # sqrt 
                                    else:
                            ...
                            if lFlag and (m1 == 0) and (m2 == 0):
                                if p < P:
                                    ...
                                    if sqrt:
                                        BBss2k *= np.dot(
                                            Bh[p][:, s1:s1+1].T/np.sqrt(L[s1]),
                                            Bh[p][:, s2:s2+1]/np.sqrt(L[s2]) # sqrt
                                        )
                                    else:
                                        BBss2k *= np.dot(
                            ...
                            if lFlag and tFlag and (m2 == 0) and (s2 == 0):
                                ...
                                if p < P:
                                    ...
                                    if sqrt:
                                        tmp = np.dot(Bh[p][:, s1:s1+1].T/np.sqrt(L[s1]), tmp) ## sqrt
                                    else:
                                ...
                                if (k == 0) and (p > 1):
                                    if p < P:
                                        ...
                                        if sqrt:                                            
                                            tmp = np.dot(
                                                Atk(Ah, m1, p, r).T,
                                                np.tile(Bh[p][:, s1:s1+1], [1, L[s1]])/np.sqrt(L[s1])
                                            )
                                        else:
                                            tmp = np.dot(
                                                Atk(Ah, m1, p, r).T,
                        ...
                        if cFlag and lFlag and (m1 == 0) and (m2 == 0) and (s2 == 0):                            
                            ...
                            if k < P:
                                ...
                                if sqrt:
                                    CBsk = np.dot(np.tile(Bh[k][:, s1:s1+1]/ np.sqrt(L[s1]), [1, L[s1]]), CBsk)
                                else:
                        ...
                        if tFlag and (m2 == 0) and (s1 == 0) and (s2 == 0):
                            ...
                            ind1 = int(np.sum(r[:m1, k]))
                            ind2 = int(np.sum(r[:m1+1, k]))
                            axesD = range(k) + [d] + range(k+1, d)
                            axesG = range(k) + [d+1] + range(k+1, d)
                        ...
                        if tFlag and (s1 == 0) and (s2 == 0):
                            ...
                            axesG1 = range(k) + [d] + range(k+1, d)
                            axesG2 = range(k) + [d+1] + range(k+1, d)
...
def refactorizeDicts(canonical_dict=None, lro_dict=None, tucker_dict=None, newC=None, newB=None, newA=None, newG=None):
    cD = None
    if canonical_dict is not None:
        cD = {
            'Rc': canonical_dict['Rc'],
...
def mvHes(x, Th, canonical_dict=None, lro_dict=None, tucker_dict=None, curvature=True, return_vector=False, sqrt=_SQRT_CLOBAL):
                        ...
                        for q in xrange(p+1):
                                ...
                                if (p == q):
                                    ...
                                    for pi, qi in [[p, q], [q, p]]:
                                        ...
                                        if pi < P:
                                            ...
                                            if sqrt:
                                                tmp1 /= np.sqrt(L[s2])*np.sqrt(L[s1])
                            ...
                            if tFlag and (s1 == 0) and (s2 == 0):
                                ...
                                if (p == 0) and (q == 0):
                                    ...
                                    #del GmGn
                                if (p == q):
                                    axesG2 = range(p) + [d+1] + range(p+1, d)
                                    axesG1 = range(p) + [d] + range(p+1, d)
                                    ...
                                    if curvature:
                                        ind1 = int(np.sum(r[:m1, p]))
                                        ind2 = int(np.sum(r[:m1+1, p]))
                                        axes = range(p) + [d] + range(p+1, d)
                                        axes2 = range(p) + [d+1] + range(p+1, d)
                                        Ahmp = Atk(Ah, m1, p, r)
                                        Ahnp = Atk(Ah, m2, p, r)
                                    ...
                                    for pi, qi in indices:
                                        ...
                                        axesG2 = range(pi) + [d+1] + range(pi+1, d)
                                        axesG1 = range(pi) + [d] + range(pi+1, d)
                                        ...
                                        if (curvature):
                                            ind1 = int(np.sum(r[:m2, pi]))
                                            ind2 = int(np.sum(r[:m2+1, pi]))
                                            axes = range(pi) + [d] + range(pi+1, d)
                                            axes2 = range(pi) + [d+1] + range(pi+1, d)
                                            Ahmq = Atk(Ah, m1, qi, r)
                                            Ahnq = Atk(Ah, m2, qi, r)
                            ...
                            if cFlag and tFlag and (s1 == 0) and (s2 == 0) and (m2 == 0):      
                                if p == q:
                                    ...
                                    sigma = range(1, p+1) + [0] + range(p+1, d)
...
def JtJ_diag(n, canonical_dict=None, lro_dict=None, tucker_dict=None, which='full', sqrt=_SQRT_CLOBAL):
    '''
    ...
    for m in xrange(max(Rt, Rl)):
        ...
        for k in xrange(d):
            if tFlag and (m < Rt):
                ...
                prodAxes = range(k) + [d] + range(k+1, d)
                prodAxes2 = range(k) + [d+1] + range(k+1, d)
            ...
            if (m == 0):
                ...
                if lFlag and (fmc is not None) and (fmc[k] is not None):
                    WtVt = np.zeros([fmc[k][0], int(np.prod(n[:k]+n[k+1:]))])
                    for s in xrange(fmc[k][1]):
    ...
    if lFlag:
        ...
        #dBk2 =
    if tFlag:
        gdiag = [np.ones([1,1]) for m in xrange(Rt)]
        adiag = []
        tmpA = [[]]*d
    for k in xrange(d):
        if tFlag or lFlag:
...
def BJprecTCD(x, canonical_dict=None, lro_dict=None, tucker_dict=None, CtC=None, AtA=None):
    ...
    if tFlag:
        A2 = copy.deepcopy(A1)
        G2 = [None]*len(G1)
    for m in xrange(Rt): # m
        if tFlag:
            GGm = G1[m].copy()
        for p in xrange(d): # m, p
            if tFlag:
                AAmk = G[m].copy()
            if m == 0: # p
                if cFlag:
            ...
            if tFlag:
                prodAxes = range(p) + range(p+1, d)
                AAmk = np.tensordot(AAmk, G[m], axes=(prodAxes, prodAxes))
                AAmk = np.linalg.pinv(AAmk)
                A2[p][:, int(np.sum(r[:m, p])) : int(np.sum(r[:m+1, p]))] = np.dot(A1[p][:, int(np.sum(r[:m, p])) : int(np.sum(r[:m+1, p]))], AAmk)
...
def backtrackArmijo(functional, x, d, g, projector=None, beta=0.9, sigma=0.5, beta_lim=1e-8, normT=1., sign=None):
    assert (beta > 0) and (beta < 1.), "backtrackArmijo: uncorrect value of beta (%.4g)" % beta
    assert (sigma > 0) and (sigma < 1.), "backtrackArmijo: uncorrect value of sigma (%.4g)" % sigma
    assert (beta_lim > 0) and (beta_lim < beta), "backtrackArmijo: uncorrect value of beta_lim (%.4g)" % beta_lim
...
def dogleg_step(gk, Hk, dk, prec=None, maxInnerIt=100, tol=1e-6):
    ...
    ykqn, infor = conjugate_gradient_method(Hk, -gk, x0=gk.copy(), iM=prec, maxitnum=maxInnerIt, tol=tol)
...
def kModeInverse(Z, k):
    ...
    iSigma = range(1, sigma[0]+1) + [0] + range(sigma[0]+1, d)
...
def alsUpdAim(i, m, T, iA, canonical_dict=None, lro_dict=None, tucker_dict=None, orthog='qr', sqrt=_SQRT_CLOBAL):
    ...
    for s in xrange(max(Rl, Rt)):
        ...
        if s == 0:
            ...
            axesT = range(i) + [d] + range(i+1, d)
            axesD = range(i) + [d+1] + range(i+1, d)
        ...
        if lFlag and (s < Rl):
            ...
            del partTB
        if (s != m) and (s < Rt):
            axesTm = range(i) + [d+2] + range(i+1, d)
            axesTs = range(i) + [d] + range(i+1, d)
            axesAs = [d, d+1]
    ...
    if blockL is None:
        if (fmc is not None) and (fmc[modeK] is not None):
            if full:
                ...
                rv = np.zeros([tmp[0].shape[0], int(sum(L))])
...
def alsUpdBi_old(i, T, canonical_dict=None, lro_dict=None, tucker_dict=None, fullModesRanks=None, sqrt=_SQRT_CLOBAL):
    ...
    if i < P:
        ...
        for s in xrange(Rl):
            ...
            for m in xrange(max(Rt, Rl)):
                if (tFlag) and (m < Rt):
                    partBT = G[m].copy()
                if (m < Rl) and (m!=s):
                    partBB = np.ones([L[m], L[s]])
                if cFlag and (m == 0):
                    partBC = np.ones([Rc, L[s]])
                for k in xrange(d):
                    if k == i:
                        if (tFlag) and (m < Rt):
        ...
        if (fmc is not None) and (fmc[i] is not None):
            ...
            WtVt = np.zeros([fmc[i][0], int(np.prod(n[:i]+n[i+1:]))])
            ind = 0
            for s in xrange(fmc[i][1]):
                #offset = n[s]
            ...
            for m in xrange(max(Rt, Rl)):
                ...
                if m == 0:
                    # part BD (always)
                    axesT = range(i) + [d] + range(i+1, d)
                    axesBD = range(i) + [d+1] + range(i+1, d)
        ...
        for s in xrange(Rl):
            ...
            if sqrt:
                res /= np.sqrt(L[s]) ## sqrt
            result.append(res.copy())
        result = np.hstack(result)
    #print np.linalg.norm(result)
    return result

def alsUpdUi_Bsi(i, T, canonical_dict=None, lro_dict=None, tucker_dict=None, fullModesRanks=None, sqrt=_SQRT_CLOBAL):
    d = T.ndim
    n = T.shape
    cFlag = canonical_dict is not None
    lFlag = lro_dict is not None
...
def alsUpdBsi(i, s, T, canonical_dict=None, lro_dict=None, tucker_dict=None, fullModesRanks=None, sqrt=_SQRT_CLOBAL):
    d = T.ndim
    n = T.shape
    cFlag = canonical_dict is not None
    lFlag = lro_dict is not None
...
def alsUpdCi(i, T, canonical_dict=None, lro_dict=None, tucker_dict=None, fullModesRanks=None, sqrt=_SQRT_CLOBAL):
    d = T.ndim
    n = T.shape
    cFlag = canonical_dict is not None
    lFlag = lro_dict is not None
    ...
    IC_stepper = np.sum((Rc)**np.arange(d-1))
    IC_stepper = int(IC_stepper)
    for m in xrange(max(Rl, Rt)):
        if lFlag and (m < Rl):
            partCB = np.ones([L[m], Rc])
        if tFlag and (m < Rt):
            partCT = G[m].copy()
        for k in xrange(d):
            if k == i:
...
def als_loop(T, sweep, canonical_dict=None, lro_dict=None, tucker_dict=None, iA=None, projector=None, sqrt=_SQRT_CLOBAL):
    d = T.ndim
    n = T.shape
    cFlag = canonical_dict is not None
    lFlag = lro_dict is not None
...
def tcd(T,
    ...
    if x0 is None:
        C, B, A, G = initializeCBT(n, canonical_param=canonical_dict, lro_param=lro_dict, tucker_param=tucker_dict, rtype='normal', normalize=True)
    else:
        if tFlag:
        ...
        if constraints['type'] == 'projected':
            if method == 'als':
                def projector(canonical_dict, lro_dict, tucker_dict, activeMode):
                    cdN, ldN, tdN = constraints['projector'](canonical_dict, lro_dict, tucker_dict, dimnums=constraints['dimnum'], vect=False, activeModes=activeMode)
                    return cdN, ldN, tdN
            else:
                def projector(z):
            ...
            if not (method in ['tr', 'rn', 'gn', 'lm']):
                raise ValueError('Current implementation of Lagrange multipliers method needs (qusi-)Newton approach, not %s method' % (method))
            ...
            if ('multInit' in constraints.keys()) and (constraints['multInit'] is not None):
                ...
                multipliersLMM = np.random.uniform(0.001, 0.005, size=(multSize))
            wholeDecParamNum = 0.
            if cFlag:
            ...
            def HesMvAL(z):
                C1, B1, A1, G1 = fvec2fcore(z[:wholeDecParamNum], n, canonical_dict, lro_dict, tucker_dict, full_result=True)
                cdN, ldN, tdN = refactorizeDicts(canonical_dict, lro_dict, tucker_dict, C1, B1, A1, G1)
                rv = constraints['hessian_matvec'](z, multipliersLMM, canonical_dict=cdN, lro_dict=ldN, tucker_dict=tdN, dimnums=constraints['dimnum'], vect=True)
                return rv
    ...
    def mv(x):
        ...
        if LagrangeMultipliersMethod:
            rv[:wholeDecParamNum] += mvHes(x[:wholeDecParamNum], T1, canonical_dict, lro_dict, tucker_dict, curvature, return_vector=True)
            rv += HesMvAL(x)
        else:
    ...
    if method == 'tr':
        ...
        if trStep == 'dogleg':
            trStepF = lambda par1, par2, par3: dogleg_step(par1, par2, par3, prec=prec, maxInnerIt=maxInnerIt, tol=tolRes*0.1)
            curvature=False
        elif trStep == 'scg':
            trStepF = lambda par1, par2, par3: scg_step(par1, par2, par3, eps=tolGrad*0.1, maxInnerIt=maxInnerIt)
    ...
    # main loop
    for sweep in xrange(maxitnum):
        if method == 'als':
            canonical_dict, lro_dict, tucker_dict, iA = als_loop(T1, sweep, canonical_dict, lro_dict, tucker_dict, iA, projector)
            ...
            p -= x
            if backtrack:
                xnew, _, _ = backtrackArmijo(functional, x, p, g, beta=0.995, sigma=0.005, beta_lim=1e-11, normT=normT, sign=None)
            if LagrangeMultipliersMethod:
            ...
            if verbose:
                print "\t Itnum: %d\t Rho: %.5f\t Delta: %.5f" % (sweep+1, rho, trDeltaK)
        ...
        if doSA:
            ...
            err = np.sqrt(2.*functional(xnew))
            coef = (flist[-1] - err) / np.linalg.norm(xnew - x)
            epk = np.random.uniform(-1, 1+np.spacing(1.), size=x.shape)
            epk *= coef/(2.*np.linalg.norm(epk))
            err2 = np.sqrt(2.*functional(xnew + epk))
            dE = (flist[-1] - err2)*normT**2.
            random_number = np.random.random()
            print "T=%.4f, dE=%.4f, cond=%.4f, r=%.4f" % (Temperature, dE, np.exp(dE / Temperature), random_number)
        ...
        if method != 'als':
            glist.append(gnorm)
        if verbose:
            print "Itnum: %d\t Fun.val.: %.5f\t G.norm: %.5f" % (sweep+1, err, gnorm)            
...
if __name__ == '__main__':
    ...
    for itRun in xrange(Nrun):
        ...
        for itAlg in xrange(len(algs)):
            ...
            if len(info['gnorm']) == 0:
                info['gnorm'] = [np.nan]
            print "Run:%d\t Alg: %s\t Time: %.2f\t Fv: %.8f\t Gv: %.8f" % (
                itRun+1, methods_names[itAlg], t1-t0, info['funval'][-1], info['gnorm'][-1]
            )
